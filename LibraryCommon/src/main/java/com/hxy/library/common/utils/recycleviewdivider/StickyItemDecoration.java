package com.hxy.library.common.utils.recycleviewdivider;
...
/**
 * 悬浮item 需要adapter的getItemViewType(m)== Constants.TYPE_LEVEL_1 否则会报错
 * huangxiaoyu
 * 2020/4/22 0022 09:49
 */
...
//            if (mStickyView.isStickyView(view)) {
            if (parent.getAdapter().getItemViewType(m) == Constants.TYPE_LEVEL_1) {
...
//            if (mStickyView.isStickyView(view)) {
    ...
    private void getStickyViewHolder(RecyclerView recyclerView) {
        ...
        mViewHolder = mAdapter.onCreateViewHolder(recyclerView, Constants.TYPE_LEVEL_1);
